...
const TrainingCard = ({element}) => {
    return(
        ...
        <Image className={styles.training_card__image} src={element.image} alt={element.institute} width={110} height={120} priority />
        <h2 className={styles.training_card__title}>{element.obtainedTitle}</h2>
        <p className={styles.training_card__paragraph}>{element.endingYear} <span>- {element.institute}</span></p>
      </article> 
    )
}
